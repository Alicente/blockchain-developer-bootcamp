...
export const myOpenOrdersSelector = createSelector(
		...
		// Filter orders by token addresse	
...
const decorateOrder = (order, tokens) => {			
	...
	if (order.tokenGive === tokens[1].address) {		
		token0Amount = order.amountGive //The amount of the Dapp we are giving	
...
const decorateFilledOrders = (orders, tokens) => {
	//Track previous order to company history
...
const tokenPriceClass = (tokenPrice, orderId, previousOrder) => {
	// Show green price id only one order exists
...
export const myFilledOrdersSelector = createSelector(
		...
		(account, tokens, orders) => {
			...
			// Filter orders for the current trading pair
			orders = orders.filter((o) => o.tokenGet === tokens[0].address || o.tokenGet === tokens[1].address)	
			orders = orders.filter((o) => o.tokenGive === tokens[0].address || o.tokenGive === tokens[1].address)			
